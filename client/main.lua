	...
	while true do
		...
		if IsControlJustPressed(0, Keys['G']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and IsPedOnFoot(ped) then
			...
			elseif distance < 20 and distance > 2.0 then
				TriggerEvent("pNotify:SendNotification", {text =_U('too_far'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })

			else
				TriggerEvent("pNotify:SendNotification", {text =_U('no_players_nearby'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
